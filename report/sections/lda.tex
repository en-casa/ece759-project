...
To do this, we employ multiclass LDA, as we have several classes (10 for MNIST and 38 for Yale B.) The class separation in this case is given by the ratio 
%
\begin{equation}
  \frac{\pmb w^T \Sigma_b \pmb w}{\pmb w \Sigma \pmb w}  
\end{equation}
...
\begin{enumerate}
	...
	\item We calculate within-class scatter matrix for each class
          ...
          %
          \begin{equation}
             \Sigma_W = \sum\limits_{i =1}^{c} (N_i - 1) \Sigma_i 
          \end{equation}
	...
	\item We also calculate the between-class scatter matrix by 
          %
          \begin{equation}
            \Sigma_B = \sum\limits_{i =1}^c \dfrac{N_i}{N} (\pmb \mu_i - \pmb \mu) (\pmb \mu_i - \pmb \mu)^T
          \end{equation}
